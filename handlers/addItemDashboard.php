...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($addAmount) || !is_numeric($addAmount)) {
        header("Location: ../views/dashboard.php?error=Enter+a+valid+amount&type=add");
        exit();
    }

    if (empty($addItem)) {
        header("Location: ../views/dashboard.php?error=Enter+a+valid+item&type=add");
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        if ($amount->execute()) {
            ...
            header("location: ../views/dashboard.php?error=error adding (expenses)&type=add");
            exit();
        }
    } else {
        header("location: ../views/dashboard.php?error=error adding (items)&type=add");
        exit();
    }
}
